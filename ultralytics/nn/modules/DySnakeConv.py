...
def autopad(k, p=None):  # kennel,padding
    if p is None:
        p = k // 2 if isinstance(k, int) else [x // x for x in k]  # auto-pad
        return p
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 128, 8, 8).cuda()
    dsconv = DySnakeConv(128, 128,3).cuda()
    output = dsconv(input)
    print(output.shape)
